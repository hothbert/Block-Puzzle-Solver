...
def bfs(goalState, seenStates, queue):  # breath first search of possible board states
    start = time.time()
    while len(queue) > 0:  # queue isn't empty, there are still states to explore
        ...
        for block in boardState.blocks:  # loop through all blocks in current state
            ...
            if checkMove(boardState, row - 1, col, width, collisionBoard, "upDown"):  # checks if block can move up
                if boardState.isNewState(counter, row - 1, col, seenStates):
                    newState = createNewState(counter, boardState, row - 1, col)
                    if newState.isSolution(goalState):  # checks if goal is reached
                        newState.printSolution()  # prints moves to get to goal from the first state
                        return True  # exits because there is no point checking the rest of the queue
                    queue.append(newState)  # else adds new state to back of queue

            if checkMove(boardState, row + length, col, width, collisionBoard, "upDown"):  # checks if block can move down
                if boardState.isNewState(counter, row + 1, col, seenStates):
                    newState = createNewState(counter, boardState, row + 1, col)
                    if newState.isSolution(goalState):
                        newState.printSolution()
                        return True
                    queue.append(newState)

            if checkMove(boardState, row, col - 1, length, collisionBoard, "leftRight"):  # checks if block can move left
            ...
            if checkMove(boardState, row, col + width, length, collisionBoard, "leftRight"):  # checks block can move right
                if boardState.isNewState(counter, row, col + 1, seenStates):
                    newState = createNewState(counter, boardState, row, col + 1)
                    if newState.isSolution(goalState):
                        newState.printSolution()
...
def main(boardFile, goalFile):
    ...
    if initialState.isSolution(goalState):
        initialState.printSolution()  # checks if goal is reached and prints moves
        return True
    ...
    return bfs(goalState, seenStates, queue)
